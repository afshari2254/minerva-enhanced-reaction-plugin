(function () {
    ...
    var GranularNavigator = function () {
        ...
        // NEW: Plugin name, version and ID changed
        this.name = 'Enhanced_Reaction_Explorer';
        this.version = '1.0.0';
        this.id = 'enhanced_reaction_explorer_v1';
    };
    ...
    GranularNavigator.prototype.register = function (minervaProxy) {
        ...
        container.innerHTML = `
            <div style="padding:15px; background:#f4f6f9; border:1px solid #ddd; font-family: 'Segoe UI', sans-serif; height: 100%; overflow-y: auto;">
                ...
                <!-- NEW: Plugin title -->
                <h4 style="margin:0 0 15px 0; color:#283593; border-bottom: 3px solid #283593; padding-bottom:5px; text-transform: uppercase;">
                    Enhanced Reaction Explorer
                </h4>

                <button id="btnMap" class="modern-btn"
                    style="width:100%; padding:12px; margin-bottom:15px; background:#283593; color:white; font-weight:bold;">
                    SYNC MAP ENTITIES
                </button>

                <div id="loaderContainer" style="display:none; text-align:center;">
                    <div class="minerva-loader"></div>
                    <p style="font-size:11px; color:#666;">
                        Querying Project BioEntities...
                    </p>
                </div>

                <select id="compDrop"
                    style="width:100%; padding:12px; margin-bottom:10px; display:none; border-radius:8px; border:2px solid #283593; font-weight:bold;">
                </select>

                <button id="btnScanArea" class="modern-btn"
                    style="width:100%; padding:12px; background:#43a047; color:white; display:none; font-weight:bold;">
                    AUDIT COMPARTMENT
                </button>

                <div id="navLog"
                    style="margin-top:15px; font-size:13px; min-height:450px; background:#fff; border:1px solid #ddd; padding:15px; border-radius:8px;">
                    Ready to Sync.
                </div>
            ...
            minervaProxy.project.data.getAllBioEntities()
                .then(function (entities) {
                    ...
                    entities.forEach(function(e) {
                        ...
                        var type =
                            (e._type || e.type || "").toLowerCase();

                        if (
                            type.indexOf("compartment") !== -1 ||
                            type.indexOf("submap") !== -1
                        ) {
                            areas[e.name] = e;
                        }
                    });
        ...
        scanBtn.onclick = function () {
            ...
            // Find entities inside compartment
            allEntities.forEach(ent => {

                var isInside =
                    (
                        ent.x >= (box.x - 5) &&
                        ent.x <= (box.x + box.width + 5) &&
                        ent.y >= (box.y - 5) &&
                        ent.y <= (box.y + box.height + 5)
                    );
                ...
                if (
                    isInside &&
                    ent.id !== box.id &&
                    type.toLowerCase().indexOf("reaction") === -1
                ) {
            ...
            var out =
                '<h3 style="color:#283593; margin:0 0 10px 0;">' +
                'Inventory: ' + name +
                '</h3>' +
                ...
                'class="entity-search" ' +
                'type="text" ' +
                'placeholder="Search Entity, for example TNF or IL23">' +
            ...
            Object.keys(typeBuckets).sort().forEach(typeName => {
                ...
                out +=
                    '<b style="text-transform:uppercase; color:#d32f2f; font-size:10px;">' +
                    typeName +
                    '</b><br/>';
                    ...
                    out += `
                        <div class="ent-badge-container">
                            ...
                            <button
                                class="ent-rxn-btn"
                                data-id="${ent.id}"
                                data-model="${ent.modelId || ent._modelId}"
                                data-type="ALIAS">
                                RXN
                            </button>
            ...
            var searchInput =
                log.querySelector('#entitySearch');
            ...
            searchInput.oninput = function () {
                ...
                log.querySelectorAll('.ent-badge-container')
            ...
            log.querySelectorAll('.ent-rxn-btn').forEach(btn => {
                ...
                btn.onclick = function() {
                    ...
                    log.innerHTML =
                        '<div class="minerva-loader"></div>' +
                        '<p align="center">Fetching Sequence...</p>';


                    minervaProxy.project.data
                                    ...
                                    <div class="rxn-card">
                                        ...
                                        <div class="flow-box">
                                            ...
                                            <div class="flow-item">
                                                ...
                                                <b>REACTION START</b><br>

                                                ${rxn._reactants.length}
                                                Input(s)
